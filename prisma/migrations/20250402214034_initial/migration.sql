...
CREATE TABLE delegate_stake_create_events (
    ...
    token_lock_ref varchar NOT NULL REFERENCES dag_token_locks(hash) ON DELETE CASCADE,
    parent_hash varchar NOT NULL,
...
CREATE INDEX delegate_stake_create_events_token_lock_ref_idx ON public.delegate_stake_create_events USING btree (token_lock_ref);
...
CREATE TABLE delegate_stake_withdraw_events (
    hash varchar PRIMARY KEY,
    source_addr varchar NOT NULL REFERENCES addresses(address) ON DELETE CASCADE,
    stake_ref varchar NOT NULL REFERENCES delegate_stake_create_events(hash) ON DELETE CASCADE,
    global_snapshot_hash varchar NOT NULL REFERENCES global_snapshots(hash) ON DELETE CASCADE,
    created_at timestamp DEFAULT now() NOT NULL,
    updated_at timestamp DEFAULT now() NOT NULL
);
...
CREATE INDEX delegate_stake_withdraw_events_stake_ref_idx ON public.delegate_stake_withdraw_events USING btree (stake_ref);
