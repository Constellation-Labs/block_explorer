...
UNION ALL
SELECT
    mst.hash, mst.source_addr, mst.amount, mst.created_at, mst.updated_at,
    'SpendTransaction', gs.hash,  gs.ordinal,
...
JOIN metagraph_snapshots ms
    ON mst.metagraph_id = ms.metagraph_id AND mst.snapshot_hash = ms.hash AND mst.currency_id is null
...
CREATE OR REPLACE VIEW dag_spend_transactions_view AS
SELECT
  null as currency_id,
  mst.hash,
  mst.source_addr,        -- from abstract_transactions
  mst.amount,             -- from abstract_transactions
  mst.created_at,         -- from abstract_transactions
  mst.updated_at,         -- from abstract_transactions
  mst.destination_addr,
  mst.allow_spend_ref,
  ms.global_snapshot_hash AS snapshot_hash
FROM metagraph_spend_transactions AS mst
JOIN metagraph_snapshots AS ms
  ON ms.hash = mst.snapshot_hash
WHERE mst.currency_id IS NULL
  AND ms.global_snapshot_hash IS NOT NULL;